...
<html lang="gl-ES">
    ...
    <body>
        ...
        <main>
                    ...
                    if(isset($_POST["imaxe"])){
                        ...
                        if(!$conexiongl){
                            ...
                            if($conexiongl->query($engadirpublicacion) == true){
                                echo "<script>alert('A entrada publicouse exitosamente.');</script>";
                            } else if($conexiongl->query($engadirpublicacion) == false){
                                echo "<script>alert('Por problemas técnicos non se puido publicar a entrada.');</script>";
                            }
                        }
                        ...
                        if(!$conexiones){
                            ...
                            if($conexiones->query($engadirpublicacion) == true){
                                ...
                                echo "<script>alert('Por problemas técnicos non se puido publicar a entrada no idioma castelán.');</script>";
                            }
                        }
                    } else {
                        $conexiongl = new mysqli("127.0.0.1", "root", "", "anpacpivaldodubragl");
            ...
            <?php
                if(isset($admin)){
                    echo "<div class='engadirpublicacion'><button id='novapublicacion'>Engadir nova publicación</button></div>";
                    echo "<div class='formulariopublicacion'>
                        <form action='".htmlentities($_SERVER["PHP_SELF"])."' method='post'>
                            <div class='cerrarformulario'><img id='close' src='Imaxes/cerrar.png' width='25' alt='close'/></div>
                            <div class='campopublicacion'>
                                <label for='titulo'>Título da publicación:</label><br/>
                                <input type='text' name='titulo' placeholder='Escriba aquí o título da publicación...'/>
                            </div>
                            <div class='campopublicacion'>
                                <label for='descricion'>Descrición da publicación:</label><br/>
                                <textarea name='descricion' placeholder='Escriba aquí a descrición da publicación...'></textarea>
                            </div>
                            <div class='campopublicacion'>
                                <label for='imaxe'>Imaxe da publicación (Opcional):</label><br/>
            ...
            <section class="blog">
                <?php
                    ...
                    if (!$conexion) {
                        ...
                        $entradasdeblog = "SELECT Título, Descrición, Imaxe, DATE_FORMAT(Data_Publicación, '%d/%m/%Y | %H:%i') 
                        AS Publicación FROM blog ORDER BY Data_Publicación DESC";
                        $resultados = $conexion->query($entradasdeblog);
                        if ($resultados->num_rows == 0) {
                            echo "<p style='color: red;'>Aínda non hai entradas publicadas.</p>";
                        } else {
                            ...
                            do {
                                ...
                                <p class='datapublicacion'>Data de publicación: {$row["Publicación"]}</p>
                                </article>";
                                $row = $resultados->fetch_assoc();
                                $i++;
                            } while ($row && $i <= 5);
