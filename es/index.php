    ...
    <body>
        <header>
            ...
            <nav class="navegacion">
                <ul>
                    <li id="inicio" class="seleccionado">INICIO</li>
        ...
        <main>
            ...
            <aside class="taboleiro1">
                <div class="anuncio1">
                    <img src="Imágenes/Anuncio1.jpg" width="100%" alt="Campamento Digital">
                </div>
                <div class="enlacesdeinterese">
                    ...
                    <ul>
                        ...
                        <li id="refugiodepistacho">Refugio de Pistacho</li>
                    </ul>
                    <script>
                        let menuescolar = document.getElementById("menuescolar");
                        let asamblea = document.getElementById("asamblea");
                        let refugiodepistacho = document.getElementById("refugiodepistacho");
                        ...
                        refugiodepistacho.addEventListener("click", function(){
                    ...
                    <div class="vercomentarios">
                        <?php
                            ...
                            if(!$conexion){
                                ...
                                $vercomentarios = "SELECT * FROM comentarios ORDER BY Fecha_Publicación DESC";
                                $resultados = $conexion->query($vercomentarios);
                                if($resultados->num_rows == 0){
                                    ...
                                    do {
                                        echo "<p><img src='Imágenes/User.png' width='30' style='vertical-align: middle;' alt='User'>&nbsp;&nbsp;&nbsp;{$row["Nombre"]} | <span><small><a href='mailto:{$row["Correo"]}'>{$row["Correo"]}</a></small></span> :</p><br/>
                                        <p>{$row["Comentario"]}</p><br/>
                                        <p><small>{$row["Fecha_Publicación"]}</small></p><hr>";
                                        $row = $resultados->fetch_assoc();
                                        $i++;
                                    } while ($row && $i <= 5);
                                    echo "</div>";
            ...
            <?php
                if(isset($_POST["titulo"]) && isset($_POST["descricion"])){
                    $titulo = $_POST["titulo"];
                    $descricion = $_POST["descricion"];
                    if(isset($_POST["imaxe"])){
                        $imaxe = $_POST["imaxe"];
                        $conexiongl = new mysqli("127.0.0.1", "root", "", "anpacpivaldodubragl");
                        if(!$conexiongl){
                            echo "<p style='color: red;'>No se pudo establecer la conexión a nuestra base de datos.</p>";
                        } else {
                            $engadirpublicacion = "INSERT INTO blog(Título, Descrición, Imaxe, Data_Publicación) 
                            VALUES('$titulo', '$descricion', 'Imaxes/Blog/$imaxe', SYSDATE())";
                            if($conexiongl->query($engadirpublicacion) == true){
                                echo "";
                            } else if($conexiongl->query($engadirpublicacion) == false){
                                echo "<script>alert('Por problemas técnicos no se pudo publicar la entrada en el idioma gallego.');</script>";
                            }
                        }
                        ...
                        if(!$conexiones){
                            ...
                            $engadirpublicacion = "INSERT INTO blog(Título, Descripción, Imagen, Fecha_Publicación) 
                            VALUES('$titulo', '$descricion', 'Imágenes/Blog/$imaxe', SYSDATE())";
                            if($conexiones->query($engadirpublicacion) == true){
                                echo "<script>alert('La entrada se ha publicado exitosamente.');</script>";
                            } else if($conexiones->query($engadirpublicacion) == false){
                                echo "<script>alert('Por problemas técnicos no se pudo publicar la entrada.');</script>";
                            }
                        }
                    } else {
                        $conexiongl = new mysqli("127.0.0.1", "root", "", "anpacpivaldodubragl");
                        if(!$conexiongl){
                            ...
                            $engadirpublicacion = "INSERT INTO blog(Título, Descripción, Imagen, Fecha_Publicación) 
                            VALUES('$titulo', '$descricion', NULL, SYSDATE())";
                            if($conexiongl->query($engadirpublicacion) == true){
                                echo "";
                            } else if($conexiongl->query($engadirpublicacion) == false){
                                echo "<script>alert('Por problemas técnicos no se pudo publicar la entrada en el idioma gallego.');</script>";
            ...
            <?php
                if(isset($admin)){
                    echo "<div class='engadirpublicacion'><button id='novapublicacion'>Añadir nueva publicación</button></div>";
                    echo "<div class='formulariopublicacion'>
                        <form action='".htmlentities($_SERVER["PHP_SELF"])."' method='post'>
                            <div class='cerrarformulario'><img id='close' src='Imágenes/cerrar.png' width='25' alt='close'/></div>
                            <div class='campopublicacion'>
                                <label for='titulo'>Título de la publicación:</label><br/>
                                <input type='text' name='titulo' placeholder='Escriba aquí el título de la publicación...'/>
                            </div>
                            <div class='campopublicacion'>
                                <label for='descricion'>Descripción de la publicación:</label><br/>
                                <textarea name='descricion' placeholder='Escriba aquí la descripción de la publicación...'></textarea>
                            </div>
                            <div class='campopublicacion'>
                                <label for='imaxe'>Imagen de la publicación (Opcional):</label><br/>
                    ...
                    echo "<script>
                        let novapublicacion = document.getElementById('novapublicacion');
                        let formulariopublicacion = document.querySelector('.formulariopublicacion');
                        let close = document.getElementById('close');
                        novapublicacion.addEventListener('click', function(){
                            novapublicacion.style.display = 'none';
                            formulariopublicacion.style.display = 'block';
                        });
                        close.addEventListener('click', function(){
                            novapublicacion.style.display = 'block';
            ...
            <section class="blog">
                <?php
                    ...
                    if (!$conexion) {
                        ...
                        $entradasdeblog = "SELECT Título, Descripción, Imagen, DATE_FORMAT(Fecha_Publicación, '%d/%m/%Y | %H:%i') 
                        AS Publicación FROM blog ORDER BY Fecha_Publicación DESC";
                        $resultados = $conexion->query($entradasdeblog);
                        if ($resultados->num_rows == 0) {
                            echo "<p style='color: red;'>Aún no hay entradas publicadas.</p>";
                        } else {
                            ...
                            do {
                                ...
                                <p class='descricionentrada'>{$row["Descripción"]}</p>
                                <img class='imaxeentrada' src='{$row["Imagen"]}' width='100%' alt='{$row["Imagen"]}'/>
                                <p class='datapublicacion'>Fecha de publicación: {$row["Publicación"]}</p>
                                </article>";
                                $row = $resultados->fetch_assoc();
                                $i++;
                            } while ($row && $i <= 5);
            ...
            <aside class="taboleiro2">
                <div class="anuncio2">
                    <img src="Imágenes/Anuncio2.jpg" width="100%" alt="Jornada Multideporte">
                </div>
            ...
            <section class="galeria">
                ...
                <?php
                    for ($i = 1; $i <= 21 ; $i++) { 
                        echo "<section class='sector'><img src='Imágenes/Gallery/Diapositiva$i.png' alt='Diapositiva$i'></section>";
                    }
                ?>
        ...
        <footer>
            ...
            <div class="idiomas">
                <ul>
                    <li><a href="../gl/index.php">Gallego</a></li>
                    <li><a href="../es/index.php">Castellano</a></li>
                </ul>
            </div>
            <div class="copyright">
                <p><b>ANPA CPI VAL DO DUBRA</b>, Powered by WordPress.com &copy;<?php echo date("Y");?>. Todos los derechtos reservados.</p>
            </div>
